...
@app.route( '/catalogue' )
def catalogue():
    set_lang( request.args.get( 'lang', default = get_lang(), type = str ) )
    return render_template( 'index.html', labels = labels_lang( lbls, get_lang() ), plys = display_modules_in_rows( mdls, 3, get_lang() ), lang = get_lang() )
...
def module():
    ...
    set_lang( request.args.get( 'lang', default = get_lang(), type = str ) )
    return render_template( 'module.html', labels = labels_lang( lbls, get_lang() ), mld = mld, lang = get_lang() )


@app.route( '/events' )
def events():
    set_lang( request.args.get( 'lang', default = get_lang(), type = str ) )
    return render_template( 'events.html', labels = labels_lang( lbls, get_lang() ), events = evts, lang = get_lang() )
